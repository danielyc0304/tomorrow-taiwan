...
- **Icons**: `lucide-react` — the only runtime dependency beyond Next and React. Import the named icon and size it with the `size` prop (`<History size={14} />`); it inherits `currentColor`, so no color prop.
