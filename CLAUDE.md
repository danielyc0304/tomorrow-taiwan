...
- **Tailwind v4**, CSS-first: there is no `tailwind.config.*`. `app/globals.css` imports Tailwind, then `./styles.css` as `layer(components)` so utilities keep winning over the design system, then an `@theme inline` block that re-exports the `styles.css` tokens a component actually needs — it is an opt-in list that grows as you use colors, so expect to add a line there before `text-neutral-800` or `leading-1.02` exists — plus the breakpoints (`sm` 560px, `md` 720px, `lg` 900px, `xl` 1080px). A trailing `@layer base` block holds the deliberate walk-backs of preflight (currently `button { background-color: revert; border: revert }`, so `.btn`-less buttons keep native chrome); put anything that fights preflight there rather than unlayered. PostCSS wiring is `@tailwindcss/postcss`; `.prettierrc` and `settings.tailwindcss.cssConfigPath` both point at `app/globals.css`, which resolves `styles.css` through the import.
- **Design system** lives in `app/styles.css` — the source of truth for the look. Color/spacing/radius/elevation/typography tokens are plain `:root` custom properties, followed by component classes (`.btn`, `.card`, `.input`, `.nav`, `.table`, `.dialog`, …). A token generates a utility (`bg-accent`, `font-heading`) only once it is re-exported through the `@theme inline` block in `globals.css`; for anything not listed there, reach for a component class or `var(--color-…)` in an arbitrary value. The component classes themselves are plain CSS in `layer(components)`, not `@utility`, so Tailwind's class list never contains them — that layer choice is deliberate (it guarantees any utility overrides them) and the cost is that ESLint has to be told about each one, see below. The same layering means `styles.css`'s `a { color: var(--color-accent-700) }` applies to every link, so a link on a colored surface needs `text-inherit` to take the surface's color back — and that also suppresses `a:hover`, so give it an explicit `hover:text-…` if the hover color matters.
- **Components** are filed by scope: `components/shared/<feature>/` for what the root layout mounts on every route (`shared/header`, `shared/footer`), `components/<route>/<section>/` for a single page's sections (`home/hero`). Inside a directory there is one file per part, each with a default export, plus a file named after the directory that composes them (`hero.tsx` → `title` + `stats`, `stats.tsx` → `stat`); that composing file's outermost element is a `<section>`. Server components by default; `"use client"` only where state or `usePathname` is needed. The root layout wraps `{children}` in a `flex-1` div inside a `min-h-dvh` flex column, which is what keeps the footer at the viewport bottom on short pages — a page component must not need its own `flex-1`. Static data a section renders (nav items, footer link groups, stat figures) sits in a module-level `const` in the composing file until it comes from a real source.
...
- **ESLint flat config** composes `eslint-config-next` core-web-vitals + typescript, then `eslint-plugin-tailwindcss` (pointed at `app/globals.css` via `settings.tailwindcss.cssConfigPath`, since there is no `tailwind.config.*` for it to find), with `eslint-config-prettier` last. `eslint.config.mjs` re-declares the default ignores because adding `globalIgnores` overrides them.
  ...
  - `tailwindcss/no-custom-classname` validates a class by asking Tailwind's design system for its order, so it only recognizes real utilities. Design-system classes must be listed in the rule's `whitelist`, whose entries are matched as anchored regexes (`^…$`), one entry per family (`btn(-(ghost|secondary))?`, `tag(-(accent|outline))?`). Extend the matching entry the first time you use another `styles.css` component class or variant.
- **Icons**: `lucide-react` — one of only two runtime dependencies beyond Next and React. Import the named icon and size it with the `size` prop (`<History size={14} />`); it inherits `currentColor`, so no color prop.
- **Forms**: `react-hook-form` — the other runtime dependency. `components/submission/form.tsx` owns `useForm` and wraps the fields in `FormProvider`, so a nested part reaches the form with `useFormContext<Inputs>()` instead of prop drilling; repeatable groups use `useFieldArray`. Any `<button>` inside a `<form>` that is not the submit button needs an explicit `type="button"` — the HTML default is `submit`, and a stray submit runs validation and lets `shouldFocusError` pull focus to the first invalid field.
- **`utils/`** holds pure helpers shared across components (`utils/format.ts` → `formatDate`), imported as `@/utils/…`. Nothing stateful or React-aware goes there.
